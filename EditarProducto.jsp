...
<html>
 ...
 <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form action="sr_productos_editar" method="post">
                            <div class="mb-3">
                                <label for="txt_id" class="form-label">Id</label>
                                <input type="text" class="form-control" id="txt_id" name="txt_id" readonly>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                <label for="StockActualPrecio" class="form-label">Precio Stock Actual</label>
                                <input type="number" step="0.01" class="form-control" id="StockActualPrecio" name="StockActualPrecio"> 
                            </div>
                            <button name="btn_modificar" id="btn_modificar" value="modificar" type="submit" class="btn btn-primary" onclick="javascript:if (!confirm('¿Quiere guardar los cambios?'))
        ...
        <div class="container">
            <h2>STOCK</h2>
            <p>Maestra de datos Productos:</p> 
                    ...
                    <tr>
                        <%
                            ...
                            for (int t = 0; t < tabla.getRowCount(); t++) {
                                out.println("<tr>");
                                out.println("<td>" + tabla.getValueAt(t, 0) + "</td>");  //en estos van los numeros de clumnas del query
        ...
        <!-- script para el evento click -->
        <script type="text/javascript">
                            $('#tbl_productos').on('click', 'tr td', function (evt) {
                                var target, id, nombre, direccion, telefono, correoelectronico, nit, estado;    //propias variables
                                target = $(event.target);
                                id = target.parent("tr").find("td").eq(0).html();
                                nombre = target.parent("tr").find("td").eq(1).html();
                                direccion = target.parent("tr").find("td").eq(2).html();
                                telefono = target.parent("tr").find("td").eq(3).html();
                                correoelectronico = target.parent("tr").find("td").eq(4).html();
                                nit = target.parent("tr").find("td").eq(5).html();
                                estado = target.parent("tr").find("td").eq(6).html();

                                $("#txt_id").val(id);
                                $("#nombre").val(nombre);
                                $("#descripcion").val(direccion);
                                $("#PrecioCompra").val(telefono);
                                $("#PrecioVenta").val(correoelectronico);  //en el parentecis va el id del label del html
                                $("#StockActual").val(nit);
                                $("#StockActualPrecio").val(estado);
                                $("#modal_producto").modal('show');  //aqui colocamos el modal id
                            });
        </script>    
